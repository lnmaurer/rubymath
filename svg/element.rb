...
module SVG
  ...
  class Marker < Group
    ...
    attr_accessor :viewBox, :markerUnits, :refX, :refY, :markerWidth, :markerHeight, :orient
    ...
    def to_s
      ...
      text << %| class="#{@class}"|               if @class
      text << %| transform="#{@transform}"|       if @transform
      text << %| viewBox="#{@viewBox}"|           if @viewBox
      text << %| markerUnits="#{@markerUnits}"|   if @markerUnits
      text << %| refX="#{@refX}"|                 if @refX
      text << %| refY="#{@refY}"|                 if @refY
      text << %| markerWidth="#{@markerWidth}"|   if @markerWidth
      text << %| markerHeight="#{@markerHeight}"| if @markerHeight
      text << %| orient="#{@orient}"|             if @orient
      text << %|>\n|
      text << @elements.collect { |element| element.to_s + "\n" }.join
      text << %|</marker>\n|
    end
